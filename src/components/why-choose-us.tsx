...
const features = [
  {
    ...
    title: 'Inovasi Utama',
    description:
      'Solusi teknologi mutakhir yang membuat Anda selalu unggul dari kompetisi',
  },
  {
    icon: Shield,
    title: 'Keamanan Enterprise',
    description:
      'Protokol keamanan tingkat bank yang melindungi data dan komunikasi Anda',
  },
  {
    icon: Headphones,
    title: 'Dukungan 24/7',
        ...
        <ScrollReveal direction='up'>
          <div className='text-center'>
            <h2 className='text-3xl font-bold lg:text-4xl'>
              Mengapa Memilih JM NETWORK?
            </h2>
            <p className='mt-4 text-lg text-muted-foreground'>
              Rasakan perbedaan dengan pendekatan inovatif dan komitmen kami terhadap
              keunggulan
            </p>
          </div>
        </ScrollReveal>
        ...
        <div className='mt-16 grid gap-8 md:grid-cols-2 lg:grid-cols-3'>
          {features.map((feature, index) => (
            <ScrollReveal key={index} direction='up' delay={index * 100}>
              <Card className='group h-full transition-all duration-300 hover:-translate-y-1 bg-white hover:shadow-lg'>
